...
const Login = () => {
  ...
  const handleGoogleSignIn = async (userType: 'user' | 'agency') => {
    setIsGoogleLoading(true);
    try {
      const user = await googleAuthService.signInWithGoogle(userType);
      toast({
                ...
                <Card className="glass-card border-blue-200/50 shadow-soft">
                  ...
                  <CardContent>
                    <form onSubmit={handleAgencyLogin} className="space-y-4">
                      ...
                      <Button 
                        type="submit" 
                        className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 transition-all duration-300 text-white font-medium py-2 text-sm shadow-soft-hover"
                      >
                      ...
                      <Button
                        type="button"
                        variant="outline"
                        onClick={() => handleGoogleSignIn('agency')}
                        disabled={isGoogleLoading}
                        className="w-full border-gray-300 hover:bg-gray-50 transition-all duration-300 text-sm"
                      >
